...
* `split_vote` now included on `vote_history` to idenitify if there was an orchestrated split to flush an idol.
* `vote_event` includes extra details such as extra vote, won (Fire challenge), etc
* `tie` to indicate if the first voting round was tie rather than it being recorded in `voted_out`
