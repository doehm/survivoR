...
Now on CRAN (v2.3.0) or Git (`r version`).
...
Race, ethnicity, and POC data isonly complete for US. `poc` has been simplified to be "POC" for where a player has a field for `race` and/or `ethnicity` for US players as per (Survivor wiki)[https://survivor.fandom.com/wiki]. All others have been left blank rather than making assumptions.
...
* `strength`: If the challenge is largely strength based e.g. Shoulder the Load.
